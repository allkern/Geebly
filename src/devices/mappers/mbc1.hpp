    ...
    namespace cart {
        ...
        public:
            ...
            u32 read(u16 addr, size_t size) override {
                ...
                if (addr >= 0xa000 && addr <= 0xbfff) { if (ram_enabled) return utility::default_mb_read(ram.data(), addr, size, 0xa000); }
            ...
            void write(u16 addr, u16 value, size_t size) override {
                if (addr >= 0xa000 && addr <= 0xbfff) {
                    if (ram_enabled) utility::default_mb_write(ram.data(), addr, value, size, 0xa000);
                    return;
                }
