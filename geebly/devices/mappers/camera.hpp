        ...
        public:
            ...
            u32 read(u16 addr, size_t size) override {
                ...
                if (addr >= 0xa000 && addr <= 0xbfff) {
                    if (cam_registers_enabled) {
                        return utility::default_mb_read(camr.data(), addr, size, 0xa000);
                    }
            ...
            void write(u16 addr, u16 value, size_t size) override {
                if (addr >= 0xa000 && addr <= 0xbfff) {
                    if (cam_registers_enabled) {
                        ...
                        utility::default_mb_write(camr.data(), addr, value, size, 0xa000);
                ...
                // Camera registers enable
                if (addr >= 0x4000 && addr <= 0x5fff) {
                    if (value & 0x10) {
                        cam_registers_enabled = true;
