                ...
                switch (event.type) {
                    ...
                    case SDL_KEYDOWN: {
                        if (event.key.keysym.sym == SDLK_d) {
                            std::ofstream dump("ppu_memory_dump", std::ios::binary);

                            dump.write((char*)current_bank->data(), current_bank->size());

                            dump.close();
                        }
                        
                        if (!event.key.repeat) joypad::keydown(event.key.keysym.sym);
                    } break;
                ...
                case 0: {
                    if (clk >= 204) {
                        ...
                        if (r[PPU_LY] == PPU_HEIGHT) {
                            ...
                            _log(debug, "vblank triggered clk=%08x", clk);
                            r[PPU_STAT] &= 0xfc;
                            r[PPU_STAT] |= 1;
